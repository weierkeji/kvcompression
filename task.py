...
{example_input}"""
    ...
    def __init__(
        ...
        self.dataset_path = "/mnt/3T_disk/chenqi1/chenqi/cold-compress/datasets/dolomites"
        super().__init__(
            prompt_template, max_tokens, hf_args=None, **kwargs
        )

        # Dolomites test split does not have references, so we will use validation split for testing
        self.test_split = "validation"

        # 暂时只使用 Rouge 和 LLM-Rouge 指标
        self.metrics = {
            "BertScore": AutoMetric.from_name("bertscore"),
            "Rouge": AutoMetric.from_name("rouge"),
            #"LLM-Rouge": AutoMetric.from_name("llm-rouge"),
    ...
    def _download(self):
        ...
        # 将数据集包装成字典格式，包含所有必要的splits
        self.dataset = {
            "train": dataset["train"] if "train" in dataset else dataset["validation"],
            "validation": dataset["validation"],
            "test": dataset["validation"]  # 使用验证集作为测试集
        }
...
{choices}"""
    ...
    def __init__(self, prompt_template=DEFAULT_PROMPT_TEMPLATE, max_tokens=1, **kwargs):
        self.dataset_path = "/mnt/3T_disk/chenqi1/chenqi/cold-compress/datasets/truthful_qa"
        super().__init__(
            prompt_template,
            max_tokens,
            hf_args=["multiple_choice"],  # 只需要指定子集名称
            **kwargs,
        )
...
class RulerQA(EvaluationTask):
    ...
    def __init__(
        ...
        self.dataset_path = "/mnt/3T_disk/chenqi1/chenqi/cold-compress/datasets/ruler/qa_2_8k"  # 修改为具体的子数据集路径
        super().__init__(
            prompt_template,
            max_tokens,
            hf_args=None,  # 不再需要指定数据集名称
            **kwargs,
        )
    ...
    def _download(self):
        ...
        try:
            self.dataset = load_dataset(
                self.dataset_path,
                data_files=None,  # 如果数据集已经按split组织好了就不需要指定
                keep_in_memory=True
            )
            ...
            print(f"- {self.dataset_path}/dataset_info.json")
            print(f"- {self.dataset_path}/train/data.arrow")
            print(f"- {self.dataset_path}/validation/data.arrow")
            raise
...
{story_start}"""
    ...
    def __init__(self, prompt_template=DEFAULT_PROMPT_TEMPLATE, **kwargs):
        ...
        max_tokens = kwargs.pop("seq_length", 8192)  # 设置默认值为8192
        if max_tokens is None:
            max_tokens = 8192  # 如果仍然是None，使用默认值
            
        self.dataset_path = "/mnt/3T_disk/chenqi1/chenqi/cold-compress/datasets/pg19-test"
        super().__init__(
            prompt_template,
    ...
    def _download(self):
        ...
        # 将数据集包装成字典格式，包含所有必要的splits
        self.dataset = {
            "train": None,  # PG19不使用训练集
            "validation": None,  # PG19不使用验证集
            "test": dataset["test"] if "test" in dataset else dataset["train"]  # 使用测试集或训练集
        }
