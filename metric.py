...
class Rouge(Metric):
    ...
    def _load_metric(self, **kwargs):
        ...
        try:
            ...
            try:
                # 如果直接加载失败，尝试使用本地缓存
                self.metric = load("/mnt/3T_disk/chenqi1/chenqi/cold-compress/metrics/rouge", local_files_only=True)
            except Exception as e2:
                # 如果本地缓存也失败，尝试使用token
                token = HfFolder.get_token()
                if token:
                    self.metric = load("rouge", token=token)
                else:
                    # 如果都失败了，抛出异常
                    raise Exception(f"Failed to load rouge metric. Errors: \n{e1}\n{e2}")
...
class Bleurt(Metric):
    ...
    def _load_metric(self, **kwargs):
        ...
        try:
            ...
            try:
                # 如果直接加载失败，尝试使用本地缓存
                self.metric = load("/mnt/3T_disk/chenqi1/chenqi/cold-compress/metrics/bleurt", local_files_only=True)
            except Exception as e2:
                # 如果本地缓存也失败，尝试使用token
                token = HfFolder.get_token()
                if token:
                    self.metric = load("bleurt", token=token)
                else:
                    # 如果都失败了，抛出异常
                    raise Exception(f"Failed to load bleurt metric. Errors: \n{e1}\n{e2}")
...
class BertScore(Metric):
    ...
    def _load_metric(self, **kwargs):
        ...
        try:
            ...
            try:
                # 如果直接加载失败，尝试使用本地缓存
                self.metric = load("/mnt/3T_disk/chenqi1/chenqi/cold-compress/metrics/bertscore", local_files_only=True)
            except Exception as e2:
                # 如果本地缓存也失败，尝试使用token
                token = HfFolder.get_token()
                if token:
                    self.metric = load("bertscore", token=token)
                else:
                    # 如果都失败了，抛出异常
                    raise Exception(f"Failed to load bertscore metric. Errors: \n{e1}\n{e2}")
