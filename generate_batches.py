...
def main(
    ...
    checkpoint_path: Path = Path(
        "/mnt/3T_disk/chenqi/models/Qwen2-7B-Instruct/model.pth"
    ),
    compile: bool = True,
    ...
    if device == "cuda":
        ...
        print(f"Initial memory after loading model: {initial_memory:.3f} GB")
    ...
    if device == "cuda":
        ...
        print(f"\nCache Memory (measured): {cache_memory:.3f} GB")
    '''
    device_sync(device=device)  # MKG
    print("\n==========\n")
    #print("GENERATION:")
    ...
    output_data = {
        "max_cache_length": 53070,
        "performance": {
            "total_seconds": total_seconds,
            "tokens_per_second": tokens_per_second,
            "decode_tokens": decode_tokens,
            "bandwidth_gb_per_sec": model_size * tokens_per_second / 1e9,
            "memory_used_gb": memory_used_gb
        },
        "detailed_performance": perf_stats,
        "kv_cache_stats": cache_stats
    }

    # Create benchmark_result directory if it doesn't exist
    result_dir = Path("benchmark_result")
    result_dir.mkdir(exist_ok=True)

    # Save to JSON file in benchmark_result directory
    output_file = result_dir / f"output_max_cache_length_{output_data['max_cache_length']}.json"
